...
class EPaperAdapter(var lifecycle: Lifecycle) : androidx.viewpager.widget.PagerAdapter() {
...
class EPaperView(parent: ViewGroup, lifecycle: Lifecycle) : FrameLayout(parent.context) {
    ...
    private val target: SimpleTarget<Drawable> by lazy {
    ...
    private fun showImage() {
        ...
        GlideApp.with(image)
                .load(url)
                .fitCenter()
                .override(image.measuredWidth, parent.measuredHeight - dp2px(36f))
                .into(target)
    }
}
